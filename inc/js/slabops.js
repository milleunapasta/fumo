...
function Slab(width, height, fs, uniforms) {
    this.uniforms = uniforms; // aggiungi questa linea per salvare le uniform
...
var SlabOps = function (shaderFiles, renderer, slabWidth, slabHeight) {
    ...
    // Creazione del render target per la velocità:
    this.velocity = new THREE.WebGLRenderTarget(this.slabSize.width, this.slabSize.height, {
        depthBuffer: false,
        stencilBuffer: false,
    ...
    // Associazione dello "slab" che usa lo shader di advect per aggiornare il campo velocità:
    this.velocity.slab = new Slab(
        this.slabSize.width,
        this.slabSize.height,
        shaderFiles.get(SLABOPS_SHADER_NAMES.advect),
        {
            gridSpec: { type: "v2", value: new THREE.Vector2(this.slabSize.width, this.slabSize.height) }
        }
    );

    this.advect.slab = new Slab(
        this.slabSize.width,
        this.slabSize.height,
        shaderFiles.get(SLABOPS_SHADER_NAMES.advect),
        this.advect.uniforms  // Usa le uniform già definite
    );
}
...
SlabOps.prototype = {
    ...
    velocitySlab: function (slab) {
        this.advect.uniforms.velocity.value = this.velocity;
        this.advect.uniforms.advected.value = this.velocity.slab.state.texture;
        this.renderer.setRenderTarget(slab.temp);
        this.renderer.render(this.advect.slab.scene, Slab.camera);
        slab.swap();
    }
